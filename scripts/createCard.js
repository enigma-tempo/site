...
function createDynamicSelect() {
  ...
  api_types.then((res) => {
    createElementChild(types, res.types);
  });
  api_classes.then((res) => {
    createElementChild(classes, res.classes);
  });
  api_subclasses.then((res) => {
    createElementChild(subclasses, res.subclasses);
  });
  api_rarities.then((res) => {
    createElementChild(rarities, res.rarities);
  });
...
async function postCard() {
  ...
  const card = {
    ...
    category: subclasses.value==0?null:subclasses.value,
    effect: effects.value==0?null:effects.value,
    params: paramsTxt,
